...
UploadImage.propTypes = {
  imageUploadName: PropTypes.string.isRequired,
  changeImageUploadName: PropTypes.func.isRequired,
  imageUploadNameRef: PropTypes.object.isRequired,
};

// UploadCode component is used specifically for the contribute page
export default function UploadImage({
  imageUploadName,
  changeImageUploadName,
  imageUploadNameRef,
}) {
  return (
    <>
      <section className="bg-prim-2 flex w-full min-h-96 px-16 py-10 flex-col gap-8 rounded-xl">
        ...
        <div className="flex flex-grow-0 gap-5 items-center">
          ...
          <label
            ...
            onChange={() => changeImageUploadName()}
            ref={imageUploadNameRef}
          >
            Upload
          </label>

          {imageUploadName && (
            <p className="text-prim-1 font-league text-xl">{imageUploadName}</p>
          )}
        </div>
